...
dfap=`basename ${DIRAP}`
$DOAS su - ${USUARIO_AP} -c "cd ${DIRAP};  echo 'Precompilando recursos'; PGSSLCERT=${PGSSLCERT} PGSSLKEY=${PGSSLKEY} RAILS_ENV=${RAILS_ENV} bin/rails assets:precompile RAILS_RELATIVE_URL_ROOT=${RAILS_RELATIVE_URL_ROOT}; echo 'Actualizando indices' ; PGSSLCERT=${PGSSLCERT} PGSSLKEY=${PGSSLKEY} bin/rails RAILS_ENV=${RAILS_ENV} sip:indices ; echo 'Iniciando unicorn'; PGSSLCERT=${PGSSLCERT} PGSSLKEY=${PGSSLKEY} SECRET_KEY_BASE=${SECRET_KEY_BASE} bundle exec unicorn_rails -c ../$dfap/config/unicorn.conf.minimal.rb  -E ${RAILS_ENV} -D "
